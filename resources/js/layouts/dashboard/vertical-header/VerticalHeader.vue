...
<template>
  <v-app-bar elevation="0" height="60">
    <v-btn
      ...
      rounded="sm"
      variant="text"
      @click.stop="store.dispatch('customizer/SET_MINI_SIDEBAR',!store.state.customizer.mini_sidebar)"
      size="small"
    >
    ...
    <v-btn
      ...
      rounded="sm"
      variant="text"
      @click.stop="store.dispatch('customizer/SET_SIDEBAR_DRAWER')"
      size="small"
    >
      <MenuFoldOutlined :style="{ fontSize: '16px' }" />
    </v-btn>
      ...
      <template v-slot:activator="{ props }">
        <v-btn class="profileBtn" variant="text" rounded="sm" v-bind="props">
          <div class="d-flex align-center">
            ...
            <h6 class="text-subtitle-1 mb-0 d-sm-block d-none">{{ store.state.auth.user.name }}</h6>
          </div>
        </v-btn>
      </template>
